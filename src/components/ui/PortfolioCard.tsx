...
// Define a simple utility function for class names
const cn = (...classes: (string | boolean | undefined)[]) => {
  return classes.filter(Boolean).join(' ');
};

// Define the PortfolioCompany type inline to reduce dependencies
type PortfolioCompany = {
  ...
  // Add catch-all for any additional properties
  [key: string]: any;
};

// Inline badge component to reduce dependencies
...
// Simplified modal for company details
const CompanyModal = ({ company, isOpen, onClose }: { company: PortfolioCompany, isOpen: boolean, onClose: () => void }) => {
        ...
        <div className="p-6 space-y-4">
          ...
          {company.logo && (
            <div className="h-24 w-24 mx-auto mb-4">
              <img src={company.logo} alt={company.name} className="object-contain h-full w-full" />
            </div>
          )}
...
const PortfolioCard: React.FC<PortfolioCardProps> = ({
  ...
  const handleActionClick = (e: React.MouseEvent, url?: string | null) => {
    e.stopPropagation();
    if (url) {
  ...
  if (variant === 'compact') {
    return (
      <>
        <Card
          className={`cursor-pointer hover:bg-card/80 transition-colors p-4 ${className}`}
  ...
  return (
    <>
      <Card
        className={`h-full p-4 ${onClick || showModal ? 'cursor-pointer' : ''} ${className}`}
